...
public class RobotoCalendarView extends LinearLayout {
    ...
    private void setUpMonthLayout() {
        ...
        if (currentCalendar.get(Calendar.YEAR) == calendar.get(Calendar.YEAR)) {
            dateTitle.setText(dateText+" "+calendar.get(Calendar.YEAR));
        } else {
        ...
        for (int i = 1; i < 43; i++) {
           ...
           dayOfTheMonthText.setBackgroundResource(android.R.color.transparent);
            ...
            if(Calendar.DAY_OF_WEEK == Calendar.SUNDAY) {
                dayOfTheMonthText.setTextColor(ContextCompat.getColor(context, R.color.dark_red_bg));
            }
            else{
    ...
    private void markDayAsSelectedDay(Calendar calendar) {
        ...
        dayOfTheMonth.setTextColor(ContextCompat.getColor(context, R.color.black));
        ...
        if (circleImage1.getVisibility() == VISIBLE) {
            DrawableCompat.setTint(circleImage1.getDrawable(), ContextCompat.getColor(context, R.color.black));
        }

        if (circleImage2.getVisibility() == VISIBLE) {
            DrawableCompat.setTint(circleImage2.getDrawable(), ContextCompat.getColor(context, R.color.black));
        }
    }
